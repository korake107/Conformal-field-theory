...
\\
まず準プライマリ場を定義する。大域的共形変換$z\rightarrow \omega(z),\bar{z}\rightarrow \bar{\omega}(\bar{z})$の下で、場$\phi(x)$が次のように変換するとき、その場を準プライマリ場と呼ぶ。
\begin{align}
    \phi'(\omega,\bar{\omega})=\qty(\dv{\omega}{z})^{-h}\qty(\dv{\bar{\omega}}{\bar{z}})^{-\bar{h}}\phi(z,\bar{z})
\end{align}
ここで、$h,\bar{h}$は場の共形次元と呼ばれる無次元数である。
\\
次に無限小変換の場合を考える。
\begin{align}
    z'&=z+\epsilon(z)\\
...
\begin{align}
    \delta_{\epsilon,\bar{\epsilon}} \phi(z,\bar{z}) &= \phi'(z,\bar{z}) - \phi(z,\bar{z})\\
    &= \phi'(z'-\epsilon(z),\bar{z}'-\bar{\epsilon}(\bar{z}) ) - \phi(z,\bar{z})\\
    &= -h\pdv{\epsilon(z)}{z}\phi(z,\bar{z}) - \bar{h}\pdv{\bar{\epsilon}(\bar{z})}{\bar{z}}\phi(z,\bar{z}) - \epsilon(z)\partial_z \phi(z,\bar{z}) - \bar{\epsilon}(\bar{z})\partial_{\bar{z}} \phi(z,\bar{z})\\
    &=-(h\phi \partial_z \epsilon + \epsilon \partial_z \phi) - (\bar{h}\phi \partial_{\bar{z}} \bar{\epsilon} + \bar{\epsilon} \partial_{\bar{z}} \phi)
\end{align}
...
このとき、2点関数は次のように変換する。
\begin{align}
    \ev{\phi_1(\lambda z)\phi_2(\lambda w)}=&\lambda^{-h_1 - h_2}\lambda^{-\bar{h}_1 - \bar{h}_2}\ev{\phi_1(z)\phi_2(w)}\\
    =&\lambda^{-h_1 - h_2}\lambda^{-\bar{h}_1 - \bar{h}_2}g(z-w)\label{101}
\end{align}
一方で、2点関数は次のようにも変換する。
\begin{align}
...
\begin{align}
    g(z-w)=\frac{C_{12}}{(z-w)^{h_1 + h_2}(\bar{z}-\bar{w})^{\bar{h}_1 + \bar{h}_2}}
\end{align}
ここで、$C_{12}$は定数である。
更に,
反転変換$z\rightarrow -\frac{1}{z},\omega\rightarrow -\frac{1}{\omega}$を考える。このとき、2点関数は次のように変換する。
\begin{align}
    \ev{\phi_1\qty(-\frac{1}{z})\phi_2\qty(-\frac{1}{w})}=&\qty(\frac{1}{z^2})^{-h_1}\qty(\frac{1}{w^2})^{-h_2}\qty(\frac{1}{\bar{z}^2})^{-\bar{h}_1}\qty(\frac{1}{\bar{w}^2})^{-\bar{h}_2}\ev{\phi_1(z)\phi_2(w)}\\
    =&\frac{(z)^{2h_1}(w)^{2h_2}(\bar{z})^{2\bar{h}_1}(\bar{w})^{2\bar{h}_2}C_{12}}{(z-w)^{h_1 + h_2}(\bar{z}-\bar{w})^{\bar{h}_1 + \bar{h}_2}}
\end{align}\label{106}
一方で、2点関数は次のようにも変換する。
\begin{align}
    \ev{\phi_1\qty(-\frac{1}{z})\phi_2\qty(-\frac{1}{w})}=&g\qty(-\frac{1}{z} + \frac{1}{w})\\
    =&g\qty(\frac{z-w}{zw})\\
    =&\frac{C_{12}}{(\frac{z-w}{zw})^{h_1 + h_2}(\frac{\bar{z}-\bar{w}}{\bar{z}\bar{w}})^{\bar{h}_1 + \bar{h}_2}}\\
    =&\frac{(z)^{h_1 + h_2}(w)^{h_1 + h_2}(\bar{z})^{\bar{h}_1 + \bar{h}_2}(\bar{w})^{\bar{h}_1 + \bar{h}_2}C_{12}}{(z-w)^{h_1 + h_2}(\bar{z}-\bar{w})^{\bar{h}_1 + \bar{h}_2}}\label{110}
\end{align}
(\ref{106})式と(\ref{110})式を見比べ、これが任意の$z,w$に対して成り立つためには、$h_1=h_2$かつ$\bar{h}_1=\bar{h}_2$が必要である。
\subsubsection{3点関数}
...
そのもとでの3点関数の変換の変換性は次のようになる。
\begin{align}
  \frac{C_{abc}}{r_{12}^ar_{23}^br_{31}^c}=&\frac{1}{}
\end{align}
プライマリ場につbseいては後で記述。
...
エネルギー運動量テンソルを複素座標の成分で表す。$dz\wedge d\bar{z}=(dx^1 +idx^2)\wedge(dx^1-idx^2) =-2idx^1\wedge dx^2
$に注意する。
\begin{align}
    \int_D d^2x \ \partial^\mu \ev{T_{\mu\nu}\epsilon^\nu (x)X}=\int_D d^2z\cdot \frac{i}{2} \ \qty(\partial^z \ev{T_{zz}\epsilon(z)X}+\partial^{\bar{z}} \ev{T_{\bar{z}z}\bar{\epsilon}(\bar{z})X}+\partial^z \ev{T_{z\bar{z}}\bar{\epsilon}(\bar{z})X}+\partial^{\bar{z}} \ev{T_{\bar{z}z}\epsilon(z)X}
    )
    \end{align}
ここで$\frac{\partial}{\partial z}=\frac{1}{2}\qty(\pdv{x^1}+\frac{1}{i}\pdv{x^2})$,$\frac{\partial}{\partial \bar{z}}=\frac{1}{2}\qty(\pdv{x^1}-\frac{1}{i}\pdv{x^2}),\epsilon(z)=\epsilon_1+i\epsilon_2,\bar{\epsilon}(\bar{z})=\epsilon_1-i\epsilon_2$と書けることと複素座標での計量に注意すると次のようにエネルギー運動量テンソルを複素座標で表すことが出来る。
\begin{align}
    T_{zz}=&\pdv{x^\mu}{z}\pdv{x^\nu}{z}T_{\mu\nu}=\qty(\frac{1}{2})\qty(\frac{1}{2})T_{11}+\qty(\frac{1}{2})
\end{align}
\begin{align}
    T_{zz}&=\frac{1}{4}\qty(T_{11}-T_{22}+iT_{12}+iT_{21})=\frac{1}{4}(T_{11}-2iT_{12}-T_{22})\\
    T_{\bar{z}\bar{z}}&=\frac{1}{4}\qty(T_{11}-T_{22}-iT_{12}-iT_{21})=\frac{1}{4}(T_{11}+2iT_{12}-T_{22})\\
    T_{z\bar{z}}&=\frac{1}{4}\qty(T_{11}+T_{22}-iT_{12}+iT_{21})=\frac{1}{4}(T_{11}+T_{22})\\
    T_{\bar{z}z}&=\frac{1}{4}\qty(T_{11}+T_{22}+iT_{12}-iT_{21})=\frac{1}{4}(T_{11}+T_{22})
\end{align}
