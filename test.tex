...
\begin{align}
 ...
 \end{align}
   ...
   \end{align}
    対称性のある変換の場合、$\delta S[\Phi]$は全微分項になる。つまり今回の場合では$\delta S[\Phi]=-x_\nu\partial^{\nu}L$となる。これを用いて保存量を求める。
\begin{align}
    0 
    &= \int d^dx \qty(\pdv{\mathcal{L}}{\Phi} - \partial_\mu\qty(\pdv{\mathcal{L}}{(\partial_\mu \Phi)}))\qty( - \frac{\delta x^\mu}{\delta \omega_a}\partial_\mu \Phi(x))\omega_a\noindent \\
    & \ \ + \int d^dx \partial_\mu\qty(\pdv{\mathcal{L}}{(\partial_\mu \Phi)}\qty(- \frac{\delta x^\mu}{\delta \omega_a}\partial_\mu \Phi(x)))\omega_a\noindent \\
    & \ \ + \int d^dx \pdv{\mathcal{L}}{(\partial_\mu \Phi)}\qty(- \frac{\delta x^\mu}{\delta \omega_a}\partial_\mu \Phi(x))\partial_\mu\omega_a\\
  &\ \  + \int d^dx \partial_\mu\qty(-x_\nu \mathcal{L})\omega_a\\
\Rightarrow \partial_\mu j^\mu_a &=0\\
j^\mu_a &= \pdv{\mathcal{L}}{(\partial_\mu \Phi)}\qty(- \frac{\delta x^\mu}{\delta \omega_a}\partial_\mu \Phi(x)) + \pdv{\mathcal{L}}{(\partial_\mu \Phi)}\qty(- \frac{\delta x^\mu}{\delta \omega_a}\partial_\mu \Phi(x)) - -x_\nu \mathcal{L}
\end{align}
...
\end{align}
これは保存の式を用いることにより、保存していることが分かる。
\begin{align}
    \frac{d}{d t}Q_a=\int d^{d-1}\partial_0j_a^0=-\int d^{d-1}x \partial_i j^i=0 
\end{align}
最後はガウスの発散定理を用いて、無限大においてカレントが0になることを使った。
...
\begin{align}
    j_\mu=T_{\mu\nu}\epsilon^\nu
\end{align}
並進は共形変換であるので、カレントの保存は次を示す。
\begin{align}
    0=\partial^\mu j_\mu=\partial^\mu(T_{\mu\nu}\epsilon^\nu)=(\partial^\mu T_{\mu \nu})\epsilon^\nu
\end{align}
$\epsilon$は任意であるので、$\partial^\mu T_{\mu\nu}=0$が得られる。一般の共形変換に対して、 $x'^\mu=x^\mu +\epsilon^\mu(x)$の場合を考える。
\begin{align}
    0=\partial^\mu j_\mu =(\partial^\mu T_{})
\end{align}
